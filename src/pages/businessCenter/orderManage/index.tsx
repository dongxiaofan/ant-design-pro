...
import SendForm from './modal/SendForm';
...
const OrderManage: React.FC<{}> = () => {
  ...
  const handleShowCreateModal = (item: any) => {
    setCreateModalTitle(item ? '编辑订单' : '新建订单')
    handleCreateModalVisible(true);
    setCurrentVal(item);
  }

  // 显示弹窗-派单
  const handleShowSendModal = (item: any) => {
  ...
  const option:any = {
    ...
    render: (text:string, record:any) => (
      <div>
        <a onClick={() => handleShowSendModal(record)}>派单</a>
  ...
  return (
    <PageHeaderWrapper>
      <Tabs animated={false} onChange={handleTabChange}>
        ...
        <TabPane tab="进行中订单" key="running"></TabPane>
        <TabPane tab="已下线订单" key="error"></TabPane>
      </Tabs>
      ...
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          <Button type="primary" onClick={() => handleShowCreateModal(null)}><PlusOutlined />发布订单</Button>
        ]}
      ...
      <SendForm
        currentVal={currentVal}
        onCancel={() => handleSendModalVisible(false)}
        showSendModal={sendModalVisible}
        query={() => searchFn(actionRef)}
      />
    </PageHeaderWrapper>
  );
