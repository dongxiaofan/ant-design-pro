...
// 表单字段
const formItem = [
  { type: 'defaultSave', label: 'ID', placeholder: '请输入ID', model: 'id', style: 'hidden' },
  { type: 'input', label: '角色名称', placeholder: '请输入角色名称', model: 'name' },
  { type: 'select', label: '数据权限', placeholder: '请选择数据权限', model: 'dataAccessRoleType', options: 'dataAccessRoleTypeList', valType: 'number' },
  { type: 'select', label: '角色分类', placeholder: '请选择角色分类', model: 'operationRoleType', options: 'operationRoleTypeList', valType: 'number' },
  { type: 'select', label: '产品类型', placeholder: '请选择产品类型', model: 'productTypeArr', options: 'productTypeSource', valType: 'number' , isMultiple: true },
...
const rules:any = {
  ...
  dataAccessRoleType: [
    { required: true, message: '请选择数据权限', whitespace: true, type: 'number' }
  ],
  permissionIdList: [
...
const CreateForm: React.FC<CreateFormProps> = props => {
  ...
  const getModel = async (id:any) => {
    ...
    if (resp.success) {
      ...
      // 菜单权限列表转小写
      var selectedNode = result.detail.permissionIdList.map((item:any) => {
        return item.toLowerCase()
      })

      formData.setFieldsValue({
        ...result.detail,
        permissionIdList: selectedNode
      });
      permissionList = tool.getTree(result.permissionList)
    }
  }
